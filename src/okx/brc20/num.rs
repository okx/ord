...
impl Num {
  ...
  // TODO check overflow
  pub fn checked_mul(&self, other: &Num) -> Result<Self, Error> {
    Ok(Self(self.0.clone() * &other.0))
  }

  pub fn checked_powu(&self, exp: u64) -> Result<Self, Error> {
    match exp {
  ...
  pub fn checked_to_u8(&self) -> Result<u8, Error> {
    if !self.0.is_integer() {
      return Err(Error::InvalidInteger(self.clone().to_string()));
    }
    self.0.clone().to_u8().ok_or(Error::Overflow)
  }
  ...
  pub fn checked_to_u128(&self) -> Result<u128, Error> {
    if !self.0.is_integer() {
      return Err(Error::InvalidInteger(self.clone().to_string()));
    }
    self
...
impl FromStr for Num {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    ...
    let num = BigDecimal::from_str(s).map_err(|_| Error::InvalidNum(s.to_string()))?;
...
impl Display for Error {
  fn fmt(&self, f: &mut Formatter) -> fmt::Result {
    match self {
      ...
      Self::InvalidInteger(n) => write!(f, "invalid integer: {}", n),
      Self::Internal(s) => write!(f, "internal error: {}", s),
      Self::InvalidNum(s) => write!(f, "invalid number: {}", s),
    }
  }
}
...
mod tests {
  ...
  fn test_checked_to_u128() {
    ...
    assert_eq!(n.checked_to_u128().unwrap_err(), Error::Overflow);

    let n = Num::from_str(&format!("{}.{}", u128::MAX - 1, "33333")).unwrap();
    assert_eq!(
      n.checked_to_u128().unwrap_err(),
      Error::InvalidInteger(n.to_string())
    );
    ...
    assert_eq!(
      ...
      Error::InvalidNum("1e2".to_string())
    );
    assert_eq!(
      Num::from_str("0e2").unwrap_err(),
      Error::InvalidNum("0e2".to_string())
    );

    assert_eq!(
      Num::from_str("100E2").unwrap_err(),
      Error::InvalidNum("100E2".to_string())
    );
  }
}
